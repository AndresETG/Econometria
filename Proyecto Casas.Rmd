...
```
```{r}
sort(table(DfTrain$median_house_value), decreasing = TRUE)
```
...
```{r}
#DfTrain <- DfTrain %>%
#  filter(median_house_value<225000)
```
...
ggplot(data = TrainNoNa, aes(x = total_bedrooms))+
  ...
  geom_density(data = TrainMean, aes(x = total_bedrooms, y = ..density..), col = 'red2', lwd = 1, linetype = 'dashed')
```
...
head(TrainUNoNa)
```
Revisamos si existen outliers de las variables uniformes

```{r}
#boxplot(TrainUNoNa[, c("population", "total_bedrooms", "total_rooms")])
```

Hay outliers para el nuevo parámetro de population

```{r}
#arrange(TrainUNoNa, desc(population),)
```

Nos deshacemos de los outliers

```{r}
#TrainUNoNa <- subset(TrainUNoNa, !(id == 19007 | id == 3365 | id == 16670 | id == 13035 | id == 12105))
#arrange(TrainUNoNa, desc(population),)
#boxplot(TrainUNoNa[, c("population", "total_bedrooms", "total_rooms")])
```
Revisamos lo mismo para total_rooms

```{r}
#arrange(TrainUNoNa, desc(total_rooms),)
```
```{r}
#rainUNoNa <- subset(TrainUNoNa, id != 1980)
#arrange(TrainUNoNa, desc(total_rooms),)
#boxplot(TrainUNoNa[, c("population", "total_bedrooms", "total_rooms")])
```
...
```{r}
boxplot(TrainUMean[, c("population", "total_bedrooms", "total_rooms")])
```
```{r}
arrange(TrainUMean, desc(population),)
```
```{r}
TrainUMean <- subset(TrainUMean, !(id == 19007 | id == 3365 | id == 16670 | id == 13035 | id == 12105))
arrange(TrainUMean, desc(population),)
boxplot(TrainUMean[, c("population", "total_bedrooms", "total_rooms")])
```
```{r}
arrange(TrainUMean, desc(total_rooms),)
```

```{r}
TrainUMean <- subset(TrainUMean, id != 1980)
arrange(TrainUMean, desc(total_rooms),)
boxplot(TrainUMean[, c("population", "total_bedrooms", "total_rooms")])
```
...
* **TrainMean:** Dataset con los Na sustituidos por la mediana de la distribución.
* **TrainUNoNa:** Dataset con las dimensionales uniformes y con los Na omitidos.
* **TrainUMean:** Dataset con las dimensionales uniformes y con los Na sustituidos por la mediana de la distribución.
* **TrainSNoNa:** Dataset estandarizado con los Na omitidos.
* **TrainSMean:** Dataset estandarizado con los Na sustituidos por la mediana de la distribución.
* **TrainSUNoNa:** Dataset estandarizado, con dimensionales uniformes y con los Na omitidos.
* **TrainSUMean:** Dataset estandarizao, con dimensionales uniformes y con los Na sustituidos por la mediana de la distribución.
...
```{r}
### Set de pruebas sin dimensionales uniformes
...
* **TestMean:** Set de datos con la mediana sustituyendo los *NA*.
* **TestUMean:** Set de datos con la mediana sustituyendo los *NA* y con las dimensionales uniformes.
* **TestSMean:** Set de datos estandarizado con la mediana sustituyendo los *NA*.
* **TestSUMean:** Set de datos estandarizado, con la mediana sustituyendo los *NA* y con dimensionales uniformes.
...
Casas.rf <- randomForest( median_house_value ~., data = ModelTrain, ntree = 40, mtry = 5, importance = TRUE, keep.forest = TRUE)
```
...
write.csv(submit1, 'xval-noOL-10-3-40t.csv', row.names = FALSE)
```
